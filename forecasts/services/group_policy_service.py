...
def update_group_policy_statistics():
    print("Group policy statistics update process begin")
    ...
    sorted_inactive_cycles = GroupPolicyInactiveCycle.objects.all().order_by('parent_id','start_date','end_date')
    ...
    while current_month_end_date <= Constants.REPORT_END_DATE:
        ...
        for group_policy in all_group_policies:
            ...
            while date <= current_month_end_date:
                if group_policy.effective_date <= date:
                    if is_inactive_on_date(sorted_inactive_cycles,group_policy.clics_db_id,date):
        ...
        report.group_policy_active_weight = total_active_weight
        report.group_policy_inactive_weight = total_inactive_weight
        report.group_policy_active_at_least_one_day = total_active_at_least_one_day
        report.save()
    ...
    print("Group policy statistics update process ended")
